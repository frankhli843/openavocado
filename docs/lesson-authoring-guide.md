...
- **Dynamic visual components are the only authoring and rendering path.** Avo
  must not use fixed registered widgets, generic declarative widgets, reusable
...
- **Multiple meaningful visual/interactive explorations when the lesson covers
  ...
  offer **at least two distinct visual perspectives**, normally several lesson
  parts or several approved `bespoke-artifact` activities. Multi-step lessons
  should prefer step-specific visuals. One thin widget for many concepts is
  rejected (enforced).
...
- **Formula strings must be KaTeX-compatible.** Keep formula values as clean
  LaTeX. Do not embed presentation wrappers such as `\colorbox`, `\fcolorbox`,
  `\bbox`, raw HTML, or CSS in formula strings. Put highlight intent in synced
  visual `active_elements`; the renderer applies safe highlighting.
