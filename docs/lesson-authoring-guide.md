...
- **Every lesson part needs a transcript and synced visual timeline.** The
  ...
  `audio.synced_visual.scene` plus `audio.synced_visual.cues`. The `scene`
  object is generated by the lesson author for that exact audio segment and
  must contain a unique `scene_id`, `title`, `motif`, `description`, and
  generated `panels[]` with visual data. The cues are timed states that cover
  most of the audio duration and change that generated scene as playback
  advances. A static transcript panel is not enough. The timeline should show
  what the part receives, what changes during the narration, and what is passed
  forward.
- **No repeated audio visuals.** Every audio segment in one lesson needs its own
  generated scene plan. Do not reuse the same panel layout, motif, cue labels,
  or scene objects with slightly different text. A Q/K/V audio should look like
  ...
  and the orientation audio should look like the local map or handoff it
  actually narrates.
- **Formal math uses LaTeX and variable definitions.** If the lesson uses
  notation such as `Attention(Q,K,V)`, `QK^T`, `softmax`, probability formulas,
...
- **Top-level audio uses the same synced pattern as lesson parts.** The first
  ...
  `start`, `end`, `label`, `headline`, `narration`, `receive`, `transform`,
  `pass`, optional `panel_id`, and optional `active_elements`, and clicking a
  cue seeks the audio to that beat. It must also include a generated `scene`
  object with unique panels for this audio segment.
