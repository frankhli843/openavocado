// Navigate, click elements whose text matches a needle (to expand sections),
// then screenshot. Usage: node cdp-expand-shot.mjs <url> <w> <h> <out> <needle...>
...
for (const needle of needles) {
  const expr = `(() => { const els=[...document.querySelectorAll('button,summary,h2,h3,[role=button],div')]; const el=els.find(e=>e.textContent&&e.textContent.includes(${JSON.stringify(needle)})&&e.offsetHeight<200); if(el){el.click();return 'clicked: '+${JSON.stringify(needle)};} return 'not found: '+${JSON.stringify(needle)}; })()`;
