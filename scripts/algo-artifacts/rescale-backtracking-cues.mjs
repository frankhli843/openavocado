// Rescale the Backtracking lesson (subject 9, seq 5) synced-visual cues +
...
// to the stored duration_hint. Mirror of rescale-trie-cues.mjs.
import Database from "better-sqlite3";

const db = new Database("data/avocadocore.db");
const row = db.prepare("SELECT id FROM lessons WHERE subject_id=9 AND sequence_number=5").get();
if (!row) { console.error("Backtracking lesson (subject 9, seq 5) not found"); process.exit(1); }
