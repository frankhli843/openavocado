// Rescale the DP-reactivation lesson (subject 9, seq 7) synced-visual cues +
...
const row = db.prepare("SELECT id FROM lessons WHERE subject_id=9 AND sequence_number=7").get();
if (!row) { console.error("DP lesson (subject 9, seq 7) not found"); process.exit(1); }
