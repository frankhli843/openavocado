// Rescale the Graph-reactivation lesson (subject 9, seq 8) synced-visual cues +
...
// to the stored duration_hint. Mirror of rescale-dp-cues.mjs.
import Database from "better-sqlite3";

const db = new Database("data/avocadocore.db");
const row = db.prepare("SELECT id FROM lessons WHERE subject_id=9 AND sequence_number=8").get();
if (!row) { console.error("DP lesson (subject 9, seq 8) not found"); process.exit(1); }
