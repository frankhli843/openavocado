// Rescale the Mixed Hard lesson (subject 9, seq 11) synced-visual cues +
...
// to the stored duration_hint. Mirror of rescale-binsearch-cues.mjs.
import Database from "better-sqlite3";

const db = new Database("data/avocadocore.db");
const row = db.prepare("SELECT id FROM lessons WHERE subject_id=9 AND sequence_number=11").get();
if (!row) { console.error("Mixed Hard lesson (subject 9, seq 11) not found"); process.exit(1); }
