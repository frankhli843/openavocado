// Rescale the Monotonic Stack lesson (subject 9, seq 6) synced-visual cues +
...
// to the stored duration_hint. Mirror of rescale-backtracking-cues.mjs.
import Database from "better-sqlite3";

const db = new Database("data/avocadocore.db");
const row = db.prepare("SELECT id FROM lessons WHERE subject_id=9 AND sequence_number=6").get();
if (!row) { console.error("Monotonic Stack lesson (subject 9, seq 6) not found"); process.exit(1); }
