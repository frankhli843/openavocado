// Rescale the System Design lesson (subject 9, seq 9) synced-visual cues +
...
// to the stored duration_hint. Mirror of rescale-graph-cues.mjs.
import Database from "better-sqlite3";

const db = new Database("data/avocadocore.db");
const row = db.prepare("SELECT id FROM lessons WHERE subject_id=9 AND sequence_number=9").get();
if (!row) { console.error("System Design lesson (subject 9, seq 9) not found"); process.exit(1); }
