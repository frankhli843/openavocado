/**
 * Backfill lesson 29 ("Evals as Infrastructure: Gbench, Ditto, and One-Shot
 * Reproducible Runs") in subject 5, replacing the deterministic local-queue
 ...
 *
 * The authored content lives in scripts/lesson-29-content/*.json so this backfill
 ...
 *
 * Usage:
 *   pnpm tsx scripts/backfill-lesson-29.ts --validate   # dry run, no writes
 *   pnpm tsx scripts/backfill-lesson-29.ts               # write (makes a backup)
 */
...
const LESSON_ID = 29;
const CONTENT_DIR = path.join(__dirname, "lesson-29-content");
...
const activities: Array<{ type: string; title: string; content: any }> = [
  { type: "audio", title: "Video: Evals as infrastructure", content: overview },
  { type: "lesson_part", title: "Part 1: Gbench, a seeded environment that scores what the model does", content: part1 },
  { type: "lesson_part", title: "Part 2: Ditto on Borg and the Borg to GCP crossover", content: part2 },
  { type: "lesson_part", title: "Part 3: One-shot reproducible runs, Terraform to Docker to Cloud Run", content: part3 },
  { type: "practice_code", title: "Integrator: the reproducible eval run controller", content: integrator },
  { type: "assessment", title: "Assessment: eval and serving infrastructure", content: assessment },
];
...
function buildGenerated(): GeneratedLessonContent {
  return {
    ...
    metadata: {
      ...
      source_context_summary: "Evals as Infrastructure, authored from subject 5 workpad seq 5 + Sara meeting + public research sources",
    },
...
function write() {
  ...
  const backup = DB_PATH.replace(/\.db$/, `.before-lesson-29-backfill-${stamp}.db`);
