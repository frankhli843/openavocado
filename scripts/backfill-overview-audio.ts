 ...
 *   AVOCADOCORE_DB_PATH=/srv/open-avocado/data/open-avocado.db pnpm tsx scripts/backfill-overview-audio.ts --no-audio
 */
...
function isPodcastOverviewScript(script: string): boolean {
  ...
  const maleTurns = script.match(/^(?:Leo|Male host|Host A|Daniel|Alex):/gim)?.length ?? 0;
