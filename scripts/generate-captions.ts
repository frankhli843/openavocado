 ...
 *
 * Env: AVOCADOCORE_RUNTIME_ROOT (default process.cwd()) — where runtime_artifacts
 * lives (point at the live tree while working in a worktree).
 *
...
const outDir = path.join(RUNTIME_ROOT, "runtime_artifacts", "videos", `lesson_${lessonId}`);
...
const relPath = path.relative(RUNTIME_ROOT, outPath);
