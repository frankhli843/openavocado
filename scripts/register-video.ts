 ...
 * Env: AVOCADOCORE_DB_PATH (live DB), AVOCADOCORE_RUNTIME_ROOT (runtime_artifacts root).
 *
...
function runtimePath(relPath: string): string {
  return path.join(RUNTIME_ROOT, relPath.replace(/^runtime_artifacts\//, ""));
}
...
const absVideo = runtimePath(relVideoPath);
...
const hasPoster = fs.existsSync(runtimePath(relPoster));
const hasCaptions = fs.existsSync(runtimePath(relCaptions));
