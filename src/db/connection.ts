...
function applyAdditiveMigrations(db: Database.Database): void {
  ...
  // SQLite. When an older DB predates 'subject.created', rebuild the table
  // inside a transaction preserving every row. Idempotent: guards on the
  // stored SQL.
  migrateNextLessonJobsTriggerEventCheck(db);
...
function migrateNextLessonJobsTriggerEventCheck(db: Database.Database): void {
  const row = db
    .prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='next_lesson_jobs'")
    .get() as { sql: string } | undefined;
  if (!row || /'subject\.created'/.test(row.sql)) return; // already allows 'subject.created'
    ...
    const rebuild = db.transaction(() => {
      ...
      db.exec(`
        INSERT INTO next_lesson_jobs__new
          (id, subject_id, completed_lesson_id, discarded_lesson_id, trigger_event, adapter, status,
           payload, adapter_ref, error, dispatched_at, completed_at, created_at, updated_at)
        SELECT id, subject_id, completed_lesson_id, discarded_lesson_id, trigger_event, adapter, status,
               payload, adapter_ref, error, dispatched_at, completed_at, created_at, updated_at
