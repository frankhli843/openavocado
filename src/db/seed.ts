...
export function seedDatabase(): void {
  ...
  // Vision preprocessing subject, exact title/level/description preserved for regression QA.
  const gdmId = (
    db
      .prepare(
        `INSERT INTO subjects (learner_id, title, description, goals, criteria, current_level) VALUES (?, ?, ?, ?, ?, ?)`
      ...
      .run(
        ...
        "Understand the full image preprocessing pipeline from raw pixel data to model-ready tensors. Reach competence-level understanding of transforms, normalization, tokenization, and batching as applied to multimodal architectures.",
        "Code-first approach using Python/PIL/NumPy. Implement each preprocessing step from scratch before using library abstractions. Connect each transform to why the model architecture requires it. Focus on multimodal input contracts.",
        "familiarity"
      )
  ).lastInsertRowid as number;
...
`,
  ...
  // Vision model image preprocessor lesson (queued).
  const gdmLesson1Id = (
    db
      .prepare(
        `INSERT INTO lessons (subject_id, title, description, status, sequence_number, goals, tags, generated_by)
      ...
      .run(
        ...
        "Trace an image from JPEG on disk through resize, normalization, channel reordering, and tokenization into the tensor format a multimodal model can consume.",
        "queued",
        1,
  ...
  insertActivity.run(
    ...
    JSON.stringify({
      ...
      blocks: [
        ...
        { type: "paragraph", text: "Models have a fixed input resolution baked into their patch embedder. A typical vision encoder expects square image tensors at a specific resolution. An arbitrary photo is unlikely to be that size, so you resize first." },
        ...
        { type: "heading", text: "Preview: tokenization and input contracts" },
        { type: "callout", tone: "info", text: "Preview only: after preprocessing, a vision encoder splits the tensor into patches and turns them into tokens, and the surrounding model runtime wraps those tokens in a model-specific input contract. This lesson introduces those ideas at a high level so you know where the pipeline leads. We will explore patch tokenization and input contracts in depth in a later lesson. They are not assessed here." },
      ],
      summary: "The preprocessing pipeline standardises shape, scale, and axis order. Each step compensates for a specific mismatch between raw media data and model expectations. Tokenization and model input contracts are previewed at a high level and explored in a later lesson.",
    })
  );
  ...
  insertActivity.run(
    ...
    JSON.stringify({
      questions: [
        ...
        { id: "gq2", text: "A vision encoder expects input shape (batch, channels, 896, 896). Your PIL image loads as (1080, 1920, 3). List the transforms needed and the shape after each one.", type: "free_text" },
  ...
  db.prepare(
    `INSERT INTO generated_artifacts (lesson_id, activity_id, artifact_type, provider, voice, duration_sec, content_hash, file_path, source_script, generated_at)
     VALUES (
       ...
       'A vision model does not see pixels the way you do... (full preprocessing-pipeline walkthrough, tokenization and model input contracts previewed at a high level and explored in a later lesson)',
       datetime('now')
     )`
...
export const IMAGE_PREPROCESSING_MC_QUIZ = {
  ...
  questions: [
    ...
    },
    {
      ...
      difficulty: "hard",
      question: "Multimodal vision-language models have a vision encoder AND a language tokenizer. Pixel preprocessing happens:",
      choices: [
...
const LESSON4_KNOWLEDGE_GRAPH = {
  ...
  nodes: [
    ...
    { id: "multimodal-tok", label: "Multimodal Tokenization", category: "concept", covered: false, preview: true, description: "Patch tokenization and model input contracts, later" },
