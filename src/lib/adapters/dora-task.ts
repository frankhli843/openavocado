...
function buildFirstLessonAcceptance(event: SubjectCreatedEvent, channel?: string): string {
  ...
  return [
    ...
    "Before authoring content, do a deliberate planning pass. Read the subject goals and criteria, inspect SQLite evidence, update or create the subject workpad, and identify the highest-value first slice of the subject. For technical subjects, especially model building, inference, quantization, model conversion, model release workflows, or open-source contribution practice, do comprehensive current research before choosing examples or package APIs. Record the source-backed findings in task notes or lesson metadata. Do not guess the current syntax, package names, keyword arguments, flags, or release workflows for external libraries or tools. If the lesson uses an external Python library, CLI, or model package, include enough documentation in code comments and teaching text that the learner can understand the relevant parameters and keywords without already knowing that package.",
    "",
