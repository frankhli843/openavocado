...
function appendTaskQualityGate(text: string): string {
...
async function createDoraTask(
  ...
  const acceptance = appendTaskQualityGate(input.acceptance);
  ...
  if (endpoint) {
    try {
      ...
      if (!res.ok) {
      return { ok: false, error: `task endpoint responded with ${res.status}` };
      }
...
function buildFirstLessonAcceptance(event: SubjectCreatedEvent, channel?: string): string {
  ...
  return [
    ...
    "Before authoring content, do a deliberate planning pass. Read the subject goals and criteria, inspect SQLite evidence, update or create the subject workpad, and identify the highest-value first slice of the subject. For technical subjects, especially model building, inference, quantization, model conversion, model release workflows, or open-source contribution practice, complete the mandatory research gate before choosing examples or package APIs. Use official docs, model cards, source repositories, academic papers, benchmark papers, standards, and parallel adjacent sources when direct courses do not exist. Record the source-backed findings in task notes or lesson metadata. Do not guess the current syntax, package names, keyword arguments, flags, or release workflows for external libraries or tools. If the lesson uses an external Python library, CLI, or model package, include enough documentation in code comments and teaching text that the learner can understand the relevant parameters and keywords without already knowing that package.",
    "",
    ...
    "",
    "For a model-building and inference track, the first lesson should help the learner understand the lifecycle from data and tokenizer choices through training, inference, quantization, packaging, and release, while still teaching one concrete part deeply enough to practice. If the learner's stated goal involves a specific open-source project or team, point toward real workflows and vocabulary without pretending the learner already knows them.",
    "",
...
/**
 * Optional dora-task compatibility adapter. This is not the default release path.
 * Operators without that task system should use webhook or agent-harness.
 */
...
/**
 * Optional dora-task regeneration adapter.
 */
...
/**
 * Optional dora-task subject.created dispatcher.
