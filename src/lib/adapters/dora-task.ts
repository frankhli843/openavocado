...
function buildFirstLessonAcceptance(event: SubjectCreatedEvent, channel?: string): string {
  ...
  return [
    ...
    "The top-level overview audio is mandatory and must be long-form: at least 15 minutes, at least 2,700 words, and written as a stand-alone two-host podcast lesson with clear male/female speaker labels such as Leo: and Maya:. Use a calm, conversational long-form interview / NotebookLM-style back-and-forth without imitating any specific living person. Leo teaches the mechanism in plain language. Maya is a curious skeptical student who challenges vague terms, asks why the concept matters, asks how the actual object changes, asks how that change helps the next prediction/decision/action, and keeps drilling until the causal chain is clear. The audio must be useful away from the screen and must not mention lesson parts, exercises, practice, assessments, or page structure. Define every major noun before using it and repeat the important ideas from more than one perspective. Do not leak authoring instructions or meta-planning phrases like 'the learner should', 'the lesson should', 'the overview should', 'the audio should', or 'the transcript should'. Speak in 'you' and 'we'. When formulas are discussed in audio, say them in words, for example 'Q times K transpose divided by the square root of d sub k', then explain what that quantity means.",
