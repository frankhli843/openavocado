 ...
 *
 *   1. edge-tts         — default learner-facing narration voice.
...
const SPEAKER_LABEL_RE =
  /(?:\*\*)?(Leo|Male host|Host A|Daniel|Alex|Maya|Female host|Host B|Guest|Ava|Mina)(?:\*\*)?\s*:\s*/gi;
...
function synthesizeEdgeDialogue(
  script: string,
  outPath: string,
  maleVoice: string,
  femaleVoice: string
...
/**
 * Learner-facing default narration via edge_tts. It writes MP3 directly, then
 * validates duration and size so a partial output never gets recorded as a
 * valid lesson artifact.
 */
function synthesizeEdge(script: string, outPath: string, voice: string): TtsResult {
  ensureDir(outPath);
 ...
 *
 * Provider order: edge-tts, OpenAI TTS (if key + quota), then espeak-ng
...
export async function synthesizeSpeech(
  ...
  const wantEdge = opts.provider === "edge-tts";
  const wantOpenAI = opts.provider === "openai-tts";
  const wantEspeak = opts.provider === "espeak-ng";
  const edgeVoice = opts.voice ?? "en-US-BrianNeural";
  ...
  if (wantEdge) {
    return synthesizeEdgeDialogue(trimmed, opts.outPath, edgeVoice, femalePodcastVoice)
      ?? synthesizeEdge(trimmed, opts.outPath, edgeVoice);
  }
  ...
  // Default cascade: use the learner-facing edge voice first.
  try {
    return synthesizeEdgeDialogue(trimmed, opts.outPath, edgeVoice, femalePodcastVoice)
      ?? synthesizeEdge(trimmed, opts.outPath, edgeVoice);
  } catch (err) {
    ...
    console.warn(
      `[tts] edge-tts unavailable, trying fallback providers: ${
