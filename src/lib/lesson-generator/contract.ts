...
export const LESSON_QUALITY_BAR_PROMPT = [
  ...
  "- TOP-LEVEL OVERVIEW AUDIO MINIMUM: the first audio activity is the learner's primary stand-alone audio lesson and must target at least 15 minutes of substantive Doraemon-voice audio, which means at least 2,700 words unless the lesson is explicitly diagnostic/reference-only and documents why it is shorter. The transcript must use a two-host podcast format with clear male and female speaker labels, for example `Leo:` and `Maya:`. Use a calm long-form interview / NotebookLM-style back-and-forth without imitating a specific living person. Leo teaches the mechanism in plain language. Maya is a curious, skeptical student voice: she challenges vague explanations, asks why the concept matters, asks how it changes the actual object, asks how that change helps the next prediction/decision/action, and keeps pressing until the causal chain is clear. The overview must not talk about lesson parts, exercises, practice, assessments, or page structure. It should work when listened to away from the screen. It must define every major noun, explain why each step matters, connect the steps, repeat the essential model from multiple perspectives, and use concrete examples in plain language. A short caption, table of contents, or quick intro fails QA.",
