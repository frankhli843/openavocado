 ...
 *
 * This file defines the stable interface between the Open Avocado app and
 ...
 *
 * Implementations:
 * - External task system: creates a ticket or job in the operator's own queue
...
/** Fifteen minutes with margin at the default edge TTS pace. */
const MIN_OVERVIEW_AUDIO_WORDS = 2700;
const PODCAST_MALE_RE = /^(?:\*\*)?(Leo|Male host|Host A|Daniel|Alex)(?:\*\*)?:/gim;
const PODCAST_FEMALE_RE = /^(?:\*\*)?(Maya|Female host|Host B|Guest|Ava|Mina)(?:\*\*)?:/gim;
const AUTHORING_LEAK_RE =
  /\b(?:the learner should|learner should|the learner needs|learner needs|the lesson should|this lesson should|the overview should|this overview should|the audio should|the transcript should|the script should|must be written as|should be written as|here is the lesson content|point\s+\d+\s*:\s*(?:lesson part|code practice|assessment|reading|interactive)|lesson part\s+"|code practice\s+"|assessment\s+".*checks whether)\b/i;
const GENERIC_LISTENING_COACH_RE =
  /\b(?:do not try to memorize|don't try to memorize|listen for the object|listen for this handoff|listen for the route|object and the handoff|before we dive into details, you need the route|building a mental map|guided conversation and less like a lecture|not rushing through a table of contents|treat these as signposts|ask four questions|what would the variable names be|what would a small input look like|what output would a test assert|if a word sounds important|pick one small object from the lesson|exact object matters less than the habit|this mechanism pass is slower on purpose|if two terms sound close)\b/i;
...
export const LESSON_QUALITY_BAR_PROMPT = [
  ...
  "- BESPOKE ARTIFACT PIPELINE (REQUIRED FOR ALL VISUALS): the DB-backed bespoke artifact pipeline is the only learner-facing interactive path. For any interactive or audio-synced visual, generate a self-contained React component (React + recharts/lucide-react only, no external fetches), store it via POST /api/visual-artifacts with a stable slug, build it via POST /api/visual-artifacts/{slug}/build, run Chrome MCP QA on the local pending sandbox URL (/api/visual-artifacts/{slug}/sandbox?qa=pending), take desktop and 390px mobile screenshots, attach QA evidence via POST /api/visual-artifacts/{slug}/qa-evidence, then approve it via POST /api/visual-artifacts/{slug}/approve. After approval, re-open /api/visual-artifacts/{slug}/sandbox and the live lesson URL. Approval requires notes/screenshot refs that explicitly mention desktop and mobile/390px evidence. Once approved, the lesson interactive JSON stores widget_type:'bespoke-artifact' and params.artifact_slug only, and audio synced visuals store artifact_slug plus cue timing. Component contract: export default function ArtifactComponent(). Allowed imports: react, react-dom, lucide-react, recharts. No fs, path, child_process, net, http, https, crypto.",
  ...
  "- TOP-LEVEL OVERVIEW AUDIO MINIMUM: the first audio activity is the learner's primary stand-alone audio lesson and must target at least 15 minutes of substantive narration, which means at least 2,700 words unless the lesson is explicitly diagnostic/reference-only and documents why it is shorter. The transcript must be a single coherent authored conversation, written all at once, not assembled by padding loops or repeated summary passes. Use a two-host podcast format with clear male and female speaker labels at the start of each turn, for example `Leo:` and `Maya:`. Never put a second speaker label inside another speaker's paragraph. Use a calm long-form interview style without imitating a specific living person. Leo teaches the mechanism in plain language. Maya is a curious, skeptical student voice: she challenges vague explanations, asks why the concept matters, asks how it changes the actual object, asks how that change helps the next prediction/decision/action, and asks 3-4 layer follow-up chains until the causal chain is clear. After the high-level frame, the transcript must go deeper into each major concept individually instead of looping over the same overview. The overview must not talk about lesson parts, exercises, practice, assessments, or page structure. It should work when listened to away from the screen. It must define every major noun, explain why each step matters, connect the steps, and use concrete examples in plain language. A short caption, table of contents, repeated loop, or quick intro fails QA.",
  ...
  "- TRANSCRIPT LLM QA REQUIRED: regex/validator checks only catch hard failures. A separate reviewer agent must read the full top-level transcript and at least one lesson-part transcript for semantic quality. The reviewer must judge whether the hosts are actually conversing, whether Maya behaves like a curious skeptical student, whether the dialogue drills into causal depth instead of accepting vocabulary, whether analogies and concrete examples clarify the exact lesson concepts, and whether the audio is useful without looking at the page. If the transcript is boring, generic, structural, repetitive, not concept-specific, refers to page sections/practice, or sounds like a lesson plan, the reviewer must send it back for revision and record the required improvements before approval.",
  ...
  "- LESSON PARTS: break normal lessons into collapsed `lesson_part` activities. Each part must contain written explanation, a per-part audio script/transcript, audio.synced_visual timed across that part's audio length, a bespoke-artifact interactive visualization, a part-specific executable code practice, and mixed reinforcement practice. Mixed practice must include select-one, select-all with some but not all correct, select-all with none correct, select-all with all correct, ordering, and written questions. For the select-all-with-none-correct case use `correct_indices: []` and do NOT author a 'None of the above' choice: the practice UI supplies a virtual none option, so an authored none renders twice and is rejected by validateLessonPartPracticeContent. For the select-all-with-all-correct case set `correct_indices` to every choice index and do NOT author an 'All of the above' choice: the practice UI supplies a virtual all option. (This is the opposite of the top-level assessment quiz, which requires a real 'None of the above' choice.) Written questions must provide actual_answer + rubric so /api/answer-judge can give immediate LLM feedback. Section done/undone buttons are a learner checklist only, available at any time, persisted in SQLite, and never used as a completion gate.",
  "- CODE IN EVERY SUB-LESSON + FINAL INTEGRATOR: every normal lesson_part must include a code field with a small runnable exercise for that part's mechanism. The lesson must also include a final practice_code activity that ties the lesson parts together. Coding sections must provide a walkthrough, at least two concrete input/output examples, a behavior visualization mapping input -> process -> output, and worked_examples with both a basic/readable full implementation and a best concise full implementation. The coding UI has Attempt and Learn modes. Attempt is the runnable editor. Learn is a no-typing comprehension ladder, not just the answer key: it must start with high-level multiple-choice about the algorithm, trace concrete input/output examples, ask select-all questions about concise-code lines, include ordering questions for the function reasoning path, and include function/data-type questions for every important helper and object. Learn mode must hide the runnable editor, Run/Submit controls, and runtime badge while showing the optional-coding notice, walkthrough, expected I/O, behavior map, comprehension questions, and reference answers clearly.",
  "- Interactives must deepen understanding, not merely display graphs. Each widget needs a learning objective, learner-controlled variable, visible consequence or contrast, and written takeaway. Prefer before/after and counterfactual interactions that show how the object changes and why the changed object matters. QA should reject any visual whose labels could be swapped and reused for an unrelated lesson.",
  ...
  '- SELECT-ALL QUIZ FORMAT IS REQUIRED: author every MC quiz question (final assessment quiz AND lesson-part quizzes) as a select-all question, phrased "Select all that are true" or "Select all that are NOT true" about one specific taught mechanism, using `correct_indices` with `allow_multiple_correct: true`. Vary the number of correct choices across the quiz (sometimes one, sometimes several, occasionally all) so the answer count is never guessable, and keep every choice a complete checkable claim grounded in the lesson, not a fragment. Single-answer `correct_index` questions are not acceptable in new or updated quizzes.',
  ...
  "- SPACED REINFORCEMENT: every new lesson must open its practice or quiz with brief retrieval of the learner's weakest or stalest prior concepts, chosen from mastery_signals and tag_difficulty_performance evidence (weak_spot, misconception, review_needed, and concepts untested for several lessons). Tag these review items with the original concept so the evidence updates the old concept, not just the new lesson. The comprehensive plan's resequencing section must state which prior concepts are being resurfaced now, which are scheduled for a later lesson, and what evidence would retire each from review.",
  ...
  "- TASK / HARNESS QA: generation must be tracked either in the operator's task system or in next_lesson_jobs with harness and QA evidence fields. Evidence required BEFORE QA: lesson id in DB, audio route HTTP 200, live URL or fresh DB screenshot (desktop 1280 + mobile 390), artifact build/approval refs, and validation output. QA must be done by a DIFFERENT reviewer agent (not the generator). The reviewer opens the lesson in a browser session, interacts with every activity, checks 390px mobile and desktop, records qa_status/qa_stage/qa_events/qa_agent_ref/qa_lesson_url plus desktop and mobile screenshot refs for production/local harness jobs, and only then marks the lesson ready. See docs/lesson-authoring-guide.md and docs/agent-task-harness.md.",
].join("\n");
