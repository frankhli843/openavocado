...
export const LESSON_QUALITY_BAR_PROMPT = [
  ...
  "- AUDIO + INTERACTIVE SIDE-BY-SIDE FOR ORIENTATION: every top-level audio orientation and every lesson_part audio segment must have a paired visual/interactive visible alongside the audio on desktop and immediately below it on mobile. For top-level audio, store this as audio content orientation_visual. For lesson parts, store it as audio.synced_visual plus the part interactive. Do not make learners finish listening before they can see the moving object being described.",
