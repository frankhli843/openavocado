...
export const LESSON_QUALITY_BAR_PROMPT = [
  ...
  "- AUDIO-SYNCED VISUAL TRANSCRIPTS ARE REQUIRED: every top-level audio activity and every lesson_part audio object must include the learner-visible transcript and a synced visual. Top-level audio stores timing metadata in orientation_visual; lesson parts store timing metadata in audio.synced_visual. The synced visual must include artifact_slug for an approved visual_artifacts component plus timed cues over the audio duration, so the artifact changes as playback advances. Cues must identify what object the section receives, what operation is happening, and what is passed forward. A transcript-only accordion, one static component, or a card that just writes the audio text is rejected.",
  "- UNIQUE BESPOKE AUDIO VISUALS: every audio-synced visual in the same lesson must be individually generated as its own approved visual_artifacts component for that exact audio segment. Do not repeat the same component source, receive-transform-pass layout, cue labels, scene objects, or visual motif with different text. Use different artifact code per audio: an attention score grid for Q/K/V, a residual-stream ledger for residual flow, an expansion/compression gate for an MLP, a pipeline map for orientation, or a logits table for output-head audio. The checker rejects duplicate synced-visual fingerprints within a lesson.",
