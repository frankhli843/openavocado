...
export const LESSON_QUALITY_BAR_PROMPT = [
  ...
  "- PER-SEGMENT MANIM VIDEO (REQUIRED OUTPUT): every audio segment (the top orientation and each lesson_part audio) must ALSO produce a rendered 3Blue1Brown-style ManimCE MP4 that accompanies the audio, with that segment's audio muxed in, formulas rendered via MathTex, and the discussed element highlighted (highlight is baked in by pinning each per-cue chunk to its cue window). The finished MP4 is registered as a generated_artifacts row artifact_type='video' plus a LessonSegmentVideo object in the activity content (content.orientation_video for the orientation audio, content.audio.video for a lesson_part), and the renderer prefers <video> over the legacy audio+cue-swapped artifact, falling back to the legacy path when no video is registered. The authoring agent MUST visually review sampled frames (start/mid/end per cue) and record machine-checkable evidence in manim/review/lesson_<id>/<activity>/review.json before registration; registration is refused without it. Pipeline, venv, quality flags, and resource limits live in knowledge/projects/avocadocore_dev.md and skills/avocadocore-lesson-authoring/SKILL.md. This runs as a required CC-authoring post-pass after audio generation. A model-only generator that cannot run the screenshot-review loop (e.g. the Gemini prodavo harness) keeps producing audio + legacy visuals and marks video pending_video; pnpm audit:videos catches any published lesson segment that ships without a registered, reviewed video.",
  ...
  "- DORA TASK / HARNESS QA: generation must be tracked either in a Dora task or in next_lesson_jobs with harness and QA evidence fields. Evidence required BEFORE QA: lesson id in DB, audio route HTTP 200, live-URL or fresh-DB screenshot (desktop 1280 + mobile 390), artifact build/approval refs, and validation output. QA must be done by a DIFFERENT reviewer agent (not the generator). The reviewer opens the lesson in Chrome MCP or an equivalent browser session, interacts with every activity, checks 390px mobile and desktop, records qa_status/qa_stage/qa_events/qa_agent_ref/qa_lesson_url plus desktop and mobile screenshot refs for prod/local harness jobs, and only then marks the lesson ready. See docs/lesson-authoring-guide.md 'Lesson generation workflow' and 'What separate QA means'.",
].join("\n");
