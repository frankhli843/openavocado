/**
 * Generate a native teaching lesson after a learner completes the
 * previous lesson. This is intentionally deterministic: the app must be able to
 * prove the subject -> lesson -> complete -> next lesson loop without relying
 * on private task infrastructure.
 */
...
export function generateNextLesson(
  ...
  const sourceContext = {
    ...
    generated_by: "openavocado-local-queue/v1",
  };
  ...
  const lessonResult = db
    .prepare(
      `INSERT INTO lessons
       ...
       VALUES (?, ?, ?, 'queued', ?, ?, ?, ?, 'openavocado-local-queue/v1', '1.0.0', ?, ?)`
    )
    .run(
      event.subject_id,
      title,
      ...
      JSON.stringify(["openavocado-native", theme.tag, concept]),
  ...
  upsertTags(db, event.subject_id, lessonId, ["openavocado-native", theme.tag, concept]);
...
function upsertWorkpad(
  ...
  const addition = [
    `## ${new Date().toISOString()} native generation`,
  ...
  if (existing) {
    db.prepare(
      `UPDATE subject_workpads
       SET content = ?, version = ?, last_updated_by = 'openavocado-local-queue/v1',
    ...
    db.prepare(
      `INSERT INTO subject_workpads
       ...
       VALUES (?, ?, ?, 'openavocado-local-queue/v1', 'lesson_completion')`
