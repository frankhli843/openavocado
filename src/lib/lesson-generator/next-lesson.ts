/**
 * Generate a native Open Avocado teaching lesson after a learner completes the
 * previous lesson. This deterministic fixture proves the subject -> lesson ->
 * complete -> next lesson loop without relying on external task infrastructure.
 */
import type Database from "better-sqlite3";
import type { LessonCompletedEvent } from "@/types";
import { buildFixtureComprehensivePlan } from "@/lib/lesson-generator/plan-template";
...
export function generateNextLesson(
  ...
  const sourceContext = {
    ...
    generated_by: "open-avocado-local-queue/v1",
  };
  ...
  const lessonResult = db
    .prepare(
      `INSERT INTO lessons
       ...
       VALUES (?, ?, ?, 'queued', ?, ?, ?, ?, 'open-avocado-local-queue/v1', '1.0.0', ?, ?)`
    )
    .run(
      event.subject_id,
      title,
      ...
      JSON.stringify(["open-avocado-native", theme.tag, concept]),
      JSON.stringify(buildDiagnostics(subjectTitle, theme.nextPrompt)),
      JSON.stringify(sourceContext),
      `Generated by the local fixture after ${event.lesson_title}; review ${reviewFocus} and advance toward ${readyFocus}.`
    );
  ...
  upsertTags(db, event.subject_id, lessonId, ["open-avocado-native", theme.tag, concept]);
...
function buildAudioScript(
  ...
  return [
    ...
    `The final questions are not busywork. They leave structured evidence for the native lesson loop, so the next lesson is generated from what happened here instead of from a fixed syllabus.`,
  ].join(" ");
}
      ...
      questions: [
        {
          ...
          question: "Select all that are true about what a useful adaptive lesson establishes first.",
          choices: [
            "It names the concrete object being learned",
            "It states what operation changes that object",
            "It starts from a long list of unrelated terminology",
            "None of the above",
          ],
          correct_indices: [0, 1],
          allow_multiple_correct: true,
          explanation:
            "The object and the operation on it anchor the lesson so actions and evidence have something to attach to. Terminology lists come later, if at all.",
          concept: reviewFocus,
          difficulty: "easy",
        },
        {
          id: `q${sequenceNumber}_evidence`,
          question: "Select all that are NOT true about the end-of-lesson diagnostics.",
          choices: [
            "They mark the lesson complete automatically",
            "They replace the need for practice",
            "They create evidence that shapes the next generated lesson",
            "None of the above",
          ],
          correct_indices: [0, 1],
          allow_multiple_correct: true,
          explanation:
            "Diagnostics only create evidence that shapes the next lesson. Completion stays a separate learner action, and practice is still required.",
          concept: readyFocus,
          difficulty: "medium",
        },
      ],
...
function upsertWorkpad(
  ...
  const addition = [
    `## ${new Date().toISOString()} native local generation`,
    ...
    "",
    buildFixtureComprehensivePlan({
      subjectTitle: event.subject_title,
      currentLevel: event.current_level,
      generatedLessonTitle: lessonTitle,
      generatedLessonId: lessonId,
      completedLessonTitle: event.lesson_title,
      completedLessonId: event.lesson_id,
      reviewFocus,
      readyFocus,
      phaseReason: event.level_progression.reason,
      nextDirective: event.level_progression.gates.find((gate) => gate.label === "Next lesson directive")?.detail,
    }),
  ].join("\n");
  const existing = db
  ...
  if (existing) {
    db.prepare(
      `UPDATE subject_workpads
       SET content = ?, version = ?, last_updated_by = 'open-avocado-local-queue/v1',
    ...
    db.prepare(
      `INSERT INTO subject_workpads
       ...
       VALUES (?, ?, ?, 'open-avocado-local-queue/v1', 'lesson_completion')`
