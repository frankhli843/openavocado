...
export const COMPREHENSIVE_LESSON_PLAN_TEMPLATE = [
  ...
  "Every lesson-generation phase must create or update a durable subject-level roadmap before authoring the lesson. This roadmap lives in the subject workpad as the mutable latest plan and is summarized in the subject journal as the append-only audit trail. It is not optional planning prose. It is the long-horizon curriculum brain for Open Avocado.",
  "",
  "Minimum length requirements:",
  "- Full roadmap update: at least 1,500 words for normal technical subjects, or at least 1,000 words for short non-technical subjects. If the subject goal is broad, such as building an LLM, contributing to an open-weights model, inference, quantization, model packaging, or frontier-paper study, use at least 2,000 words.",
  ...
  "- References: at least 5 references for technical subjects. Prefer official docs, model cards, source repositories, academic papers, standards, textbooks, course syllabi, high-quality public educational material, and prior Open Avocado evidence. If direct courses are unavailable, use parallel academic and implementation sources that teach the same mechanism or evaluation problem. If live research is unavailable in a deterministic local fixture, say that clearly and list internal evidence references instead.",
  "",
  ...
  "Local deterministic fixtures must still write this plan shape, but may label live research as unavailable and use internal evidence references. Production harnesses and external task workers must use tool-capable research and include external references when the topic requires current knowledge.",
].join("\n");
...
export function buildFixtureComprehensivePlan(params: {
  ...
  return [
    ...
    "",
    `Updated: ${generatedAt}`,
    `Generator: open-avocado-local-queue/v1 deterministic fixture`,
    `Subject: ${params.subjectTitle}`,
    `Current phase: ${params.currentLevel}`,
    "",
    "## 1. Subject Goal And Learner Outcome",
    `${params.subjectTitle} should become a coherent, usable learning path rather than a sequence of isolated generated lessons. The learner should be able to explain the subject at a high level, identify the important objects and operations, practice those operations, and eventually transfer the ideas into real work. Because this update came from the deterministic local queue, it records the durable planning structure that Open Avocado expects, while clearly marking that deep external research and semantic phase judgment should be supplied by the AI harness when available.`,
    "",
    ...
    "- External references: not available in deterministic fixture mode. A production harness or external task worker must replace this section with official docs, papers, syllabi, or other verified references before shipping a research-sensitive lesson.",
    "",
    ...
    "This fixture cannot browse, so it records the reference categories the production harness must fill in: official framework or model documentation, relevant papers, high-quality course syllabi, implementation guides, and prior Open Avocado lessons. For an LLM-building subject, the reference map should compare several sources: a model architecture explanation, a training or fine-tuning guide, tokenizer documentation, inference or serving docs, and a quantization or model packaging source. The plan should adapt that source sequence to the learner's evidence rather than copying any syllabus blindly.",
    "",
    ...
    "",
    "Milestone D, applied contribution path. This milestone must become a 1,000+ word section in the AI harness plan. It should connect the subject to the learner's practical goal, such as contributing to an open-source project or understanding model-building work. It should include contribution workflows, release-card reading, reproducibility checks, model artifact inspection, and the difference between learning demos and production constraints. The AI should consult current official docs and project-specific references before authoring lessons here.",
    "",
    ...
    "- Required future external references: official docs, model cards, source repositories, academic papers, benchmark papers, course maps, standards, or implementation references relevant to the exact subject. If no direct course exists, use parallel sources from adjacent domains and explain why each source transfers.",
    "",
