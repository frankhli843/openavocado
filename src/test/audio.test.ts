 ...
 *  - synthesizeSpeech default provider produces edge-tts audio when
...
const HAS_EDGE_TTS = edgeTtsAvailable();
...
describe.runIf(HAS_EDGE_TTS)("synthesizeSpeech (default edge-tts voice)", () => {
  it("uses edge-tts by default for learner-facing lesson audio", async () => {
    ...
    expect(res.provider).toBe("edge-tts");
