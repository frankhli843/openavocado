...
describe("MarkdownText", () => {
  ...
  it("renders headings and safe markdown links", () => {
    render(<MarkdownText text={"# Agent Notes\n\nSee [model docs](https://example.com/model-docs) for context."} />);

    const heading = screen.getByRole("heading", { name: "Agent Notes" });
    expect(heading.tagName).toBe("H3");

    const link = screen.getByRole("link", { name: "model docs" });
    expect(link).toHaveAttribute("href", "https://example.com/model-docs");
  });
